<?php
function _curl($url, $request = null, $request_type = _HTTP_GET, $timeout = 10, $return_header = FALSE, $header = array(), $referer = '', $user_agent = '', $cookie = '', $cookiejar = FALSE, $cookie_file = null) {
	$curl_handler = curl_init ();
	is_array ( $header ) ? curl_setopt ( $curl_handler, CURLOPT_HEADER, $header ) : curl_setopt ( $curl_handler, CURLOPT_HEADER, array ($header ) );
	if (_HTTP_POST === $request_type) {
		curl_setopt ( $curl_handler, CURLOPT_URL, $url );
		curl_setopt ( $curl_handler, CURLOPT_POST, 1 );
		curl_setopt ( $curl_handler, CURLOPT_POSTFIELDS, $request );
	} else
		($request != null) ||($request != '')  ? $url = $url . '?' . $request : FALSE;
		curl_setopt ( $curl_handler, CURLOPT_URL, $url );
	...
	if (curl_errno ($curl_handler)) return curl_errno ($curl_handler);
